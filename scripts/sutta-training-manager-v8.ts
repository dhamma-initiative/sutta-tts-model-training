...
async function installPythonEnv() {
  ...
  if (!envExist) {
    ...
    if (isDryRun) {
      console.log("[DRY-RUN] Would run: " + MAMBA_BIN + " create -y -r /root/micromamba -n py311 python=3.11.9 pip -c conda-forge");
    } else {
...
async function runPipeline() {
  ...
  if (args.includes("--init")) {
    ...
    console.log("Usage: deno run --allow-all sutta-training-manager-v8.ts [--init | --train | --monitor] [--dry-run] [--diag-setup]");
  }
}
