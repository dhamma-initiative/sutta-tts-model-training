# =================================================================
# SUTTAPLAYER ONNX EXPORT & QUANTIZATION PIPELINE (v1.2.0)
# =================================================================
...
try:
    ...
    print(f"❌ ImportError: Failed to import onnxruntime or its components.")
    print(f"   Details: {e}")
    print("\n👉 Troubleshooting Steps:")
    print("   1. Verify onnxruntime is installed: pip install onnxruntime")
    print("   2. Verify onnx is installed: pip install onnx")
    print("   3. Check your active python environment vs system environment.")
    sys.exit(1)
...
def generate_piper_json_sidecar(config_path, output_json_path, phoneme_map_path=None):
    ...
    piper_config = {
        "audio": {
            "sample_rate": vits_config.get("audio", {}).get("sample_rate", 22050)
        },
        "espeak": {
            "voice": vits_config.get("espeak", {}).get("voice", "en-gb")
        },
        "inference": {
        ...
        "phoneme_type": vits_config.get("phoneme_type", "text"),
        "phoneme_map": {},
        "phoneme_id_map": {},
        "num_symbols": vits_config.get("num_symbols", 256),
    ...
    if phoneme_map_path and os.path.exists(phoneme_map_path):
        ...
        print("📋 Using phoneme_id_map from training config.")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to your custom phoneme_map.json"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    fp32_onnx_path = os.path.join(args.output_dir, "en_gb-suttaplayer-medium_fp32.onnx")
    quant_onnx_path = os.path.join(args.output_dir, "en_gb-suttaplayer-medium_quant.onnx")
    ...
    # 🚨 CRITICAL FIX: Use sys.executable instead of hardcoded "python3" to inherit Colab's active paths!
    export_cmd = [
    ...
    try:
        ...
        print(f"✅ Quantized model created at: {quant_onn_path}\n")
