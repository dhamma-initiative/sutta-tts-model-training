...
async function installPythonEnv() {
  ...
  if (!envExist) {
    ...
    if (isDryRun) {
      console.log("[DRY-RUN] Would run: ./bin/micromamba create -y -r /root/micromamba -n py311 python=3.11.9 pip -c conda-forge");
    } else {
...
async function cloneRepo() {
  ...
  if (isDryRun) {
    console.log("[DRY-RUN] Would run inside " + repoDir + ": ./bin/micromamba " + pipInstall.join(" "));
  } else {
  ...
  if (isDryRun) {
    console.log("[DRY-RUN] Would run inside " + repoDir + ": ./bin/micromamba " + buildAlign.join(" "));
  } else {
    try {
