# Google Colab Training Guide & Google Sheets Integration (v5)
...
2. **Colab Google Sheets Sync Cell**: A separate cell in your Jupyter notebook (`sutta-piper-trainer-v5.ipynb.txt`) uses the VM's native Google OAuth token to securely read this CSV from Google Drive and push row additions asynchronously to a designated Google Sheet [sutta-piper-trainer-v5.ipynb.txt].
...
When the notebook's Step 7 cell launches, it will securely establish **`SuttaPlayer_UAT_Convergence`** as a Google Sheet in your Drive [sutta-piper-trainer-v5.ipynb.txt].
